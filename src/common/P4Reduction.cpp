...
void P4Reduction<realT, derotFunctObj, verboseT>::setupConfig( mx::app::appConfigurator &config )
{
    ...
    config.add( "psfResponse.analyticBatchSize",
                "",
                "psfResponse.analyticBatchSize",
                mx::app::argType::Required,
                "psfResponse",
                "analyticBatchSize",
                false,
                "size_t",
                "Maximum analytic source measurements sharing baseline factors (default 32; memory limited)" );
...
void P4Reduction<realT, derotFunctObj, verboseT>::loadConfig( mx::app::appConfigurator &config )
{
    ...
    config( m_psfAnalyticBatchSize, "psfResponse.analyticBatchSize" );
...
void P4Reduction<realT, derotFunctObj, verboseT>::validateConfiguration() const
{
    ...
    static_cast<void>( psfSamplingModeString( m_psfSamplingMode ) );
    ...
    if( m_psfAnalyticBatchSize == 0 )
    {
        throw mx::exception<verboseT>( mx::error_t::invalidconfig, "psfResponse.analyticBatchSize must be positive" );
    }
    ...
    if( m_psfSamplingMode == P4PSFSamplingMode::refitDifference )
    {
                                                          ...
                                                          const std::vector<realT> &maximumRadii )
{
    ...
    m_psfAnalyticFactorCount = 0;
    m_psfAnalyticRealizedBatchSize = 0;
    ...
    std::optional<imageT> psfTemplate;
    ...
    {
        if( totalSearchPixels > std::numeric_limits<std::size_t>::max() - statistics.searchPixelCount )
        {
            throw mx::exception<verboseT>( mx::error_t::sizeerr, "P4 total search-pixel count overflow" );
        }
    ...
    if( calculatePSF )
    {
        m_localPSFResponseRequired.resize( m_regionStatistics.size() );
        for( std::size_t region = 0; region < m_regionStatistics.size(); ++region )
        {
        ...
        if( sharedPSF && !psfSamplingGrid.radii.empty() )
        {
            ...
            {
                throw mx::exception<verboseT>( mx::error_t::sizeerr,
                                               "P4 sparse local-response search count exceeds lookup range" );
            }
            ...
            std::vector<RadialPSFSource> allSources;
            allSources.reserve( totalSearchPixels );
            std::vector<RadialPSFSource> availableSources;
            availableSources.reserve( totalSearchPixels );
            std::vector<std::pair<std::size_t, std::size_t>> sourceOwners;
            sourceOwners.reserve( totalSearchPixels );
                ...
                for( std::size_t search = 0; search < grids[region].searchPixelCount(); ++search )
                {
                        ...
                        m_psfSampleAvoidRadius > 0 )
                    {
                        const double avoidanceSquared = static_cast<double>( m_psfSampleAvoidRadius ) *
                                                        static_cast<double>( m_psfSampleAvoidRadius );
                        for( std::size_t planet = 0; planet < this->m_planetSep.size() && !avoided; ++planet )
                        {
                            const double skyAngle = mx::math::dtor( -static_cast<double>( this->m_planetPA[planet] ) );
                            for( const double angle : derotationAngles )
                            {
                                const double detectorAngle = skyAngle + angle;
                                const double planetRow =
                                    grids.front().xCenter() +
                                    static_cast<double>( this->m_planetSep[planet] ) * std::sin( detectorAngle );
                                const double planetColumn =
                                    grids.front().yCenter() +
                                    static_cast<double>( this->m_planetSep[planet] ) * std::cos( detectorAngle );
                                const double deltaRow = source.row - planetRow;
                                const double deltaColumn = source.column - planetColumn;
                                if( deltaRow * deltaRow + deltaColumn * deltaColumn <= avoidanceSquared )
                                {
                                    avoided = true;
                                    ++m_psfSampleExcludedCount;
                                    break;
                                }
                            }
                        }
                    ...
                    sourceOwners.emplace_back( region, search );
                    ++globalSearch;
                }
            }
            ...
            if( m_psfSamplingMode == P4PSFSamplingMode::detectorLocal )
            {
                requiredSearch.reserve( m_psfMeasurementSamples.size() );
                for( const RadialPSFSample &sample : m_psfMeasurementSamples )
                {
                    requiredSearch.push_back( sample.sourceIndex );
                }
                std::sort( requiredSearch.begin(), requiredSearch.end() );
                requiredSearch.erase( std::unique( requiredSearch.begin(), requiredSearch.end() ),
                                      requiredSearch.end() );
            }
            else if( m_psfSamplingMode == P4PSFSamplingMode::skyExact )
            {
                std::vector<std::pair<double, double>> sampledCoordinates;
                sampledCoordinates.reserve( m_psfMeasurementSamples.size() );
                for( const RadialPSFSample &sample : m_psfMeasurementSamples )
                {
                    const RadialPSFSource &source = allSources.at( sample.sourceIndex );
                    sampledCoordinates.emplace_back( source.row, source.column );
                }
                std::vector<double> psfDerotationAngles( static_cast<std::size_t>( this->m_Nims ), 0 );
                if( this->m_doDerotate )
                {
                    for( int image = 0; image < this->m_Nims; ++image )
                    {
                        const double angle =
                            static_cast<double>( this->m_derotF.derotAngle( static_cast<std::size_t>( image ) ) );
                        if( !mx::math::isFinite( angle ) )
                        {
                            throw mx::exception<verboseT>( mx::error_t::invalidarg,
                                                           "P4 PSF output derotation angles must be finite" );
                        }
                        psfDerotationAngles[static_cast<std::size_t>( image )] = angle;
                    }
                }
                const reconstructorT planner( this->m_Nrows,
                                              this->m_Ncols,
                                              grids.front().xCenter(),
                                              grids.front().yCenter(),
                                              m_psfStampSize,
                                              m_localPSFRows,
                                              m_localPSFColumns );
                requiredSearch = planner.requiredSearchIndices( searchIndex, sampledCoordinates, psfDerotationAngles );
            }
            for( std::vector<std::uint8_t> &required : m_localPSFResponseRequired )
            {
                std::fill( required.begin(), required.end(), 0 );
            }
            for( const std::size_t required : requiredSearch )
            {
                if( required >= sourceOwners.size() )
                {
                    throw mx::exception<verboseT>( mx::error_t::sizeerr,
                                                   "P4 required local-response index is outside search geometry" );
                }
                const std::pair<std::size_t, std::size_t> &owner = sourceOwners[required];
                m_localPSFResponseRequired[owner.first][owner.second] = 1;
            }
            const std::size_t requiredCount = requiredSearch.size();
            std::cerr << "P4 sparse local PSF responses: " << requiredCount << " / " << totalSearchPixels
                      << " search pixels required by " << m_psfMeasurementSamples.size() << " measurements";
            if( m_psfSamplingMode == P4PSFSamplingMode::detectorLocal )
            {
            ...
            {
                std::cerr << " using paired finite-amplitude refits; " << m_psfSampleExcludedCount
                          << " candidates avoided near known planets";
            }
            std::cerr << '\n';
        }
            ...
            for( std::size_t search = 0; search < searchPixelCount; ++search )
            {
                ...
                if( attempt )
                {
                    try
                    {
                        ...
                        const bool responseRequired = sharedPSF && m_localPSFResponseRequired[region][search] != 0;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                fitValid = fitDetectorSearch( result,
                                                              predictors,
                                                              target,
                                                              responseRequired ? &coefficients : nullptr,
                                                              nullptr,
                                                              grid,
                                                              search,
                                                              temporalSelections,
...
#pragma omp critical(P4ReductionStatistics)
    ...
    if( !compactFinalization )
    {
        ...
        if( result == 0 && processPSF )
        {
            ...
            processPSFProducts( grids, *psfModel, *psfTemplate, regionExclusions, finalImagePath, finalHeader );
    ...
    const int combinedImageSize = m_automaticDerotationSize != 0 ? m_automaticDerotationSize : this->m_Nrows;
    ...
    P4PCA::workspaceT &workspace ) const
{
    ...
    const auto &predictors = basis.predictors();
    const auto &target = basis.target();
    P4PCA::matrixT source( this->m_Nims, grid.predictorCount() );
    P4PCA::vectorT targetSource( this->m_Nims );
    ...
    P4PCA::calculateResponse( analytic, basis, source, targetSource );
...
template <typename realT, class derotFunctObj, class verboseT>
void P4Reduction<realT, derotFunctObj, verboseT>::calculateAnalyticSamples(
    std::vector<std::vector<imageT>> &responses,
    std::vector<std::vector<psfValidityT>> &validities,
    const std::vector<pixelGridT> &grids,
    const imageT &psfTemplate,
    const std::vector<RadialPSFSample> &samples,
    const std::vector<double> &derotationAngles,
    HCI::combine responseCombineMethod,
    realT responseSigmaThreshold )
{
    P4LocalGeometry::lookupImageT searchLookup =
        P4LocalGeometry::lookupImageT::Constant( this->m_Nrows, this->m_Ncols, -1 );
    std::vector<std::size_t> offsets;
    std::vector<std::pair<std::size_t, std::size_t>> searches;
    for( std::size_t region = 0; region < grids.size(); ++region )
    {
        offsets.push_back( searches.size() );
        for( std::size_t search = 0; search < grids[region].searchPixelCount(); ++search )
        {
            const auto &coordinate = grids[region].searchPixel( search ).coordinate();
            searchLookup( coordinate.row(), coordinate.column() ) = search;
            searches.emplace_back( region, search );
        }
    }
    ...
    const long double frames = this->m_Nims, modes = m_modeFractions.size();
    const long double pixels = static_cast<long double>( m_psfStampSize ) * m_psfStampSize;
    long double largestWorker{ 0 };
    int requestedWorkers = omp_get_max_threads();
    for( std::size_t region = 0; region < grids.size(); ++region )
    {
        const long double predictors = grids[region].predictorCount(), dimension = std::min( frames, predictors );
        largestWorker = std::max( largestWorker,
                                  1.25L * sizeof( double ) *
                                          ( 8 * frames * predictors + 10 * frames * dimension +
                                            10 * dimension * dimension + 10 * frames * modes + 80 * dimension ) +
                                      1024 * 1024 );
        requestedWorkers = std::min( requestedWorkers, std::max( 1, m_regionStatistics[region].effectiveWorkerCount ) );
    }
    // Bound simultaneous geometry, dependency allocation, residuals, source images, and consumer lookup storage.
    const long double perSample =
        1.25L *
            ( frames * pixels *
    ...
    const long double retained =
        ...
        m_psfModelBytes + m_psfReconstructionBytes +
        ( pixels * ( sizeof( realT ) + sizeof( std::uint8_t ) ) + sizeof( P4ResponseStatistics ) +
          ( 4 + P4ResponseStatistics::count ) * sizeof( realT ) ) *
            modes * samples.size();
    std::size_t batchSize = std::min( m_psfAnalyticBatchSize, samples.size() );
    if( m_memoryBudgetBytes != 0 )
    {
        while( batchSize > 1 &&
               retained + perSample * batchSize + largestWorker * requestedWorkers > m_memoryBudgetBytes )
        {
            --batchSize;
        }
        if( retained + perSample * batchSize + largestWorker > m_memoryBudgetBytes )
        {
            throw mx::exception<verboseT>( mx::error_t::allocerr,
                                           "one analytic P4 response batch exceeds the memory budget" );
        }
        requestedWorkers =
            memoryLimitedWorkerCount( requestedWorkers,
                                      m_memoryBudgetBytes,
                                      static_cast<std::size_t>( std::ceil( retained + perSample * batchSize ) ),
                                      static_cast<std::size_t>( std::ceil( largestWorker ) ) );
    }
    m_psfAnalyticRealizedBatchSize = batchSize;
    const std::vector<float> scales( this->m_Nims, 1 );
    using localResidualT = Eigen::Array<realT, Eigen::Dynamic, Eigen::Dynamic>;
    for( std::size_t first = 0; first < samples.size(); first += batchSize )
    {
        ...
        {
            const auto [region, search] = searches.at( samples[first + sample].sourceIndex );
            const auto &coordinate = grids[region].searchPixel( search ).coordinate();
            const double row = coordinate.row(), column = coordinate.column();
            const double dr = row - grids.front().xCenter(), dc = column - grids.front().yCenter();
            geometries[sample].configure( this->m_Nrows,
                                          this->m_Ncols,
                                          m_psfStampSize,
                                          row,
                                          column,
                                          derotationAngles,
                                          this->m_doDerotate,
                                          m_ownership.template cast<std::int64_t>(),
                                          searchLookup );
            sources[sample].configure( psfTemplate,
                                       this->m_Nrows,
                                       this->m_Ncols,
                                       m_psfStampSize,
                                       derotationAngles,
            ...
            {
                const auto &entry = requests[request];
...
#pragma omp parallel for schedule(static) num_threads(requestedWorkers)
        ...
        {
            if( !consumers[index].empty() )
                work.push_back( index );
        }
        const int workers = std::min( requestedWorkers, std::max( 1, static_cast<int>( work.size() ) ) );
        std::cerr << "P4 analytic response batch " << first + 1 << '-' << first + count << " / " << samples.size()
                  << ": " << work.size() << " baseline factors, workers " << workers << '\n';
        std::atomic<bool> failed{ false };
        std::exception_ptr workerException;
        // clang-format off
#pragma omp parallel num_threads(workers)
        ...
        {
            P4PCA::workspaceT workspace;
            P4PCAResponseBasis basis;
            P4PCA::matrixT predictors, response;
            P4PCA::vectorT target( this->m_Nims );
            std::vector<std::uint8_t> supported;
            std::vector<std::vector<P4ResponseStatistics>> statistics(
                count,
                std::vector<P4ResponseStatistics>( m_modeFractions.size() ) );
            std::size_t factors{ 0 }, refits{ 0 };
            double seconds{ 0 };
            // clang-format off
#pragma omp for schedule(dynamic)
            // clang-format on
            for( std::size_t item = 0; item < work.size(); ++item )
            {
                if( failed.load( std::memory_order_acquire ) )
                    continue;
                try
                {
                    ...
                    const std::size_t index = work[item];
                    const auto [region, search] = searches[index];
                    const auto &grid = grids[region];
                    if( !grid.searchPixel( search ).valid() )
                        continue;
                    const auto &coordinate = grid.searchPixel( search ).coordinate();
                    predictors.resize( this->m_Nims, grid.predictorCount() );
                    for( int frame = 0; frame < this->m_Nims; ++frame )
                    {
                        const auto image = this->m_tgtIms.image( frame );
                        target( frame ) = checkedPredictorPromotion( image( coordinate.row(), coordinate.column() ) );
                        for( std::size_t predictor = 0; predictor < grid.predictorCount(); ++predictor )
                            predictors( frame, predictor ) =
                                checkedPredictorPromotion( grid.sample( image, search, predictor ) );
                    }
                    P4PCA::prepareResponse( basis,
                                            predictors,
                                            target,
                                            m_realizedModes[region],
                                            m_rankTolerance,
                    ...
                    ++factors;
                    for( const auto &[sample, requestIndex] : consumers[index] )
                    {
                        refits += calculateAnalyticDetectorResponse( response,
                                                                     supported,
                                                                     statistics[sample],
                                                                     grid,
                                                                     search,
                        ...
                        for( std::size_t mode = 0; mode < m_modeFractions.size(); ++mode )
                        {
                            ...
                            {
                                residuals[sample][requestIndex]( frame, mode ) =
                                    checkedResidualCast( response( request.frames()[frame], mode ) );
                                validity[sample][requestIndex]( frame, mode ) = 1;
                            }
                        }
                    }
                ...
                {
                    // clang-format off
#pragma omp critical(P4AnalyticBatchException)
                    // clang-format on
                    {
                        if( !workerException )
                            workerException = std::current_exception();
            ...
            // clang-format off
#pragma omp critical(P4AnalyticBatchStatistics)
            // clang-format on
            {
                m_psfAnalyticFactorCount += factors;
                m_psfRefitDifferenceFitCount += refits;
                m_timing.psfWorkerSeconds += seconds;
                for( std::size_t sample = 0; sample < count; ++sample )
                    for( std::size_t mode = 0; mode < m_modeFractions.size(); ++mode )
                        for( std::size_t diagnostic = 0; diagnostic < P4ResponseStatistics::count; ++diagnostic )
                            m_psfResponseStatistics[mode][first + sample].counts[diagnostic] +=
                                statistics[sample][mode].counts[diagnostic];
            }
        }
        if( workerException )
            std::rethrow_exception( workerException );
        for( std::size_t sample = 0; sample < count; ++sample )
        {
            const std::size_t sampleIndex = first + sample;
            const auto &geometry = geometries[sample];
            const auto &localResiduals = residuals[sample];
            const auto &localValidities = validity[sample];
            for( std::size_t output = 0; output < m_modeFractions.size(); ++output )
            {
                P4PSFReconstructor::cubeT frames;
                P4PSFReconstructor::cubeT frameValidities;
                frames.resize( m_psfStampSize, m_psfStampSize, this->m_Nims );
                frames.setZero();
                frameValidities.resize( m_psfStampSize, m_psfStampSize, this->m_Nims );
                frameValidities.setZero();
                for( int stampColumn = 0; stampColumn < m_psfStampSize; ++stampColumn )
                {
                    for( int stampRow = 0; stampRow < m_psfStampSize; ++stampRow )
                    {
                        for( std::size_t frame = 0; frame < static_cast<std::size_t>( this->m_Nims ); ++frame )
                        {
                            const P4LocalOutputSample &outputSample =
                                geometry.outputSample( stampRow, stampColumn, frame );
                            if( !outputSample.valid() )
                            {
                                continue;
                            }
                            realT value{ 0 };
                            bool valid{ true };
                            for( const P4LocalResidualSample &dependency : outputSample.samples() )
                            {
                                if( localValidities[dependency.requestIndex()](
                                        static_cast<Eigen::Index>( dependency.frameOffset() ),
                                        static_cast<Eigen::Index>( output ) ) == 0 )
                                {
                                    valid = false;
                            ...
                            if( valid )
                            {
                                frames.image( static_cast<int>( frame ) )( stampRow, stampColumn ) = value;
                                frameValidities.image( static_cast<int>( frame ) )( stampRow, stampColumn ) = 1;
                            }
                        }
                    }
                }
                P4PSFReconstructor::combineFrames( responses[output][sampleIndex],
                                                   validities[output][sampleIndex],
                                                   frames,
                                                   frameValidities,
                                                   responseCombineMethod,
                                                   this->m_comboWeights,
    ...
    realT responseSigmaThreshold )
{
    ...
    for( std::size_t region = 0; region < grids.size(); ++region )
    {
        if( !regionExclusions[region].empty() ||
            m_temporalSelections[region].size() != static_cast<std::size_t>( this->m_Nims ) )
        {
            throw mx::exception<verboseT>(
                mx::error_t::invalidconfig,
                "P4 refit-difference response initially requires in-sample fits over every target frame" );
        }
        for( std::size_t target = 0; target < m_temporalSelections[region].size(); ++target )
        {
            const std::vector<int> &selection = m_temporalSelections[region][target];
            if( selection.size() != 1 || selection[0] != static_cast<int>( target ) )
            {
                throw mx::exception<verboseT>(
                    mx::error_t::invalidconfig,
                    "P4 refit-difference response initially requires p4.numberImages=0 and ordered target rows" );
            }
        }
    }
    ...
    if( analytic )
    {
        calculateAnalyticSamples( responses,
                                  validities,
                                  grids,
                                  psfTemplate,
                                  samples,
    ...
    P4LocalGeometry::lookupImageT searchIndexLookup =
        P4LocalGeometry::lookupImageT::Constant( this->m_Nrows, this->m_Ncols, -1 );
    std::vector<P4PixelCoordinate> sourceCoordinates;
    ...
    if( sourceCoordinates.empty() )
    {
        throw mx::exception<verboseT>( mx::error_t::sizeerr,
                                       "P4 refit-difference response has no detector search coordinates" );
    }
    ...
    for( std::size_t sampleIndex = 0; sampleIndex < samples.size(); ++sampleIndex )
    {
        ...
        if( sample.sourceIndex >= sourceCoordinates.size() )
        {
            throw mx::exception<verboseT>( mx::error_t::sizeerr,
                                           "P4 refit-difference response sample index exceeds search geometry" );
        }
        const P4PixelCoordinate &sourceCoordinate = sourceCoordinates[sample.sourceIndex];
        const double sourceRow = static_cast<double>( sourceCoordinate.row() );
        const double sourceColumn = static_cast<double>( sourceCoordinate.column() );
        const double separation = std::hypot( sourceRow - centerRow, sourceColumn - centerColumn );
        const double positionAngle =
            -std::atan2( sourceRow - centerRow, sourceColumn - centerColumn ) * 180.0 / std::numbers::pi;
        ...
        geometry.configure( this->m_Nrows,
                            ...
                            m_ownership.template cast<std::int64_t>(),
                            searchIndexLookup );
        P4TrialSource positiveSource;
        P4TrialSource negativeSource;
        positiveSource.configure( psfTemplate,
                                  this->m_Nrows,
                                  this->m_Ncols,
                                  m_psfStampSize,
                                  derotationAngles,
        ...
        for( std::size_t request = 0; request < geometry.searchRequests().size(); ++request )
        {
            const Eigen::Index frameCount =
                static_cast<Eigen::Index>( geometry.searchRequests()[request].frames().size() );
            localResiduals[request].resize( frameCount, static_cast<Eigen::Index>( m_modeFractions.size() ) );
            localResiduals[request].setZero();
            localValidities[request].resize( frameCount, static_cast<Eigen::Index>( m_modeFractions.size() ) );
...
#pragma omp for schedule(static)
            // clang-format on
            for( std::size_t requestIndex = 0; requestIndex < geometry.searchRequests().size(); ++requestIndex )
            {
                if( failed.load( std::memory_order_acquire ) )
                {
                    continue;
                ...
                try
                {
                    ...
                    double temporalSamplingSeconds{ 0 };
                    P4PCATiming timing;
                    const bool positiveValid = fitDetectorSearch( positiveResult,
                                                                  predictors,
                                                                  target,
                                                                  nullptr,
                                                                  nullptr,
                    ...
                    ++threadFitCount;
                    timing = P4PCATiming{};
                    const bool negativeValid = fitDetectorSearch( negativeResult,
                                                                  predictors,
                                                                  target,
                                                                  nullptr,
                                                                  nullptr,
                    ...
                    for( std::size_t output = 0; output < m_modeFractions.size(); ++output )
                    {
                        for( std::size_t frameOffset = 0; frameOffset < request.frames().size(); ++frameOffset )
                        {
                            const Eigen::Index frame = static_cast<Eigen::Index>( request.frames()[frameOffset] );
                            if( !positiveResult.sampleSupported( frame, output ) ||
                                !negativeResult.sampleSupported( frame, output ) )
                            {
                                continue;
                            }
                            const double difference =
                                ( positiveResult.residuals( frame, static_cast<Eigen::Index>( output ) ) -
                                  negativeResult.residuals( frame, static_cast<Eigen::Index>( output ) ) ) *
                                inverseDifference;
                            localResiduals[requestIndex]( static_cast<Eigen::Index>( frameOffset ),
                                                          static_cast<Eigen::Index>( output ) ) =
                                checkedResidualCast( difference );
                            localValidities[requestIndex]( static_cast<Eigen::Index>( frameOffset ),
                                                           static_cast<Eigen::Index>( output ) ) = 1;
                        }
                    }
                    threadResponseSeconds += omp_get_wtime() - responseBegin;
                ...
                {
                    // clang-format off
#pragma omp critical(P4RefitDifferenceException)
                    // clang-format on
                    {
                        if( !workerException )
                        {
            ...
            // clang-format off
#pragma omp critical(P4RefitDifferenceTiming)
            // clang-format on
            {
                responseWorkerSeconds += threadResponseSeconds;
        ...
        if( workerException )
        {
            ...
            catch( ... )
            {
                std::throw_with_nested( mx::exception<verboseT>(
                    mx::error_t::exception,
                    "P4 refit-difference response measurement failed at sample " + std::to_string( sampleIndex ) ) );
            }
        }
        m_timing.psfWorkerSeconds += responseWorkerSeconds;
        if( responseFitCount > std::numeric_limits<std::size_t>::max() - m_psfRefitDifferenceFitCount )
        {
            throw mx::exception<verboseT>( mx::error_t::sizeerr, "P4 refit-difference response fit count overflow" );
        }
        m_psfRefitDifferenceFitCount += responseFitCount;
        ...
        for( std::size_t output = 0; output < m_modeFractions.size(); ++output )
        {
            ...
            for( int stampColumn = 0; stampColumn < m_psfStampSize; ++stampColumn )
            {
                for( int stampRow = 0; stampRow < m_psfStampSize; ++stampRow )
                {
                    for( std::size_t frame = 0; frame < static_cast<std::size_t>( this->m_Nims ); ++frame )
                    {
                        const P4LocalOutputSample &outputSample = geometry.outputSample( stampRow, stampColumn, frame );
                        if( !outputSample.valid() )
                        {
                            continue;
                        }
    ...
    const fitsHeaderT &finalHeader )
{
    ...
    const bool sharedStateValid =
        m_localPSFModels.size() == grids.size() && m_localPSFTemporalCoefficients.size() == grids.size() &&
        m_localPSFValidity.size() == grids.size() && m_localPSFResponseRequired.size() == grids.size();
    ...
    const int finalRows = m_automaticOutputSize != 0 ? m_automaticOutputSize : this->m_Nrows;
    const int finalColumns = m_automaticOutputSize != 0 ? m_automaticOutputSize : this->m_Ncols;
    if( m_psfFilter && ( this->m_finim.rows() != finalRows || this->m_finim.cols() != finalColumns ||
    ...
    const HCI::combine responseCombineMethod =
        this->m_combineMethod == HCI::combine::sigmaMean ? HCI::combine::mean : this->m_combineMethod;
    const realT responseSigmaThreshold =
        responseCombineMethod == HCI::combine::sigmaMean ? this->m_sigmaThreshold : static_cast<realT>( 0 );
    ...
    // Publish source coordinates in the final science image, including its automatic crop.
    const int coordinateOriginRow = m_automaticOutputSize != 0 ? ( this->m_Nrows - m_automaticOutputSize ) / 2 : 0;
    const int coordinateOriginColumn = m_automaticOutputSize != 0 ? ( this->m_Ncols - m_automaticOutputSize ) / 2 : 0;
    ...
    std::size_t localResponseSearchCount{ 0 };
    for( const std::vector<std::uint8_t> &required : m_localPSFResponseRequired )
    {
        localResponseSearchCount += static_cast<std::size_t>(
            std::count_if( required.begin(), required.end(), []( std::uint8_t value ) { return value != 0; } ) );
    }
    ...
    const auto productHeader = [&]( const std::string &product, std::size_t output )
    {
        ...
        if( analyticApproximation )
        {
            header.template append<std::string>( "P4 PSF ANALYTIC FACTOR COUNT",
                                                 std::to_string( m_psfAnalyticFactorCount ),
                                                 "FP64 baseline factorizations across source batches" );
            header.template append<std::string>( "P4 PSF ANALYTIC BATCH SIZE",
                                                 std::to_string( m_psfAnalyticRealizedBatchSize ),
                                                 "maximum realized source measurements per batch" );
        }
        if( differentiatedApproximation )
        {
            header.template append<std::string>( "P4 PSF REFIT FIT COUNT",
                                                 ...
                                                 "positive-plus-negative detector regressions" );
        }
        header.template append<std::string>(
        ...
        header.template append<std::string>( "P4 SCIENCE COMBINATION",
        ...
        header.template append<std::string>( "P4 LOCAL PSF RESPONSE SEARCH COUNT",
                                             std::to_string( localResponseSearchCount ),
                                             "search pixels with calculated local responses" );
    ...
    if( m_psfFilter )
    {
        ...
        filtered.resize( finalRows, finalColumns, outputCount );
        filterNormalization.resize( finalRows, finalColumns, outputCount );
        filterSupport.resize( finalRows, finalColumns, outputCount );
        filterValidity.resize( finalRows, finalColumns, outputCount );
    ...
    for( std::size_t firstOutput = 0; firstOutput < m_modeFractions.size(); firstOutput += modeBatchSize )
    {
        ...
        for( std::size_t batchOutput = 0; batchOutput < currentBatchSize; ++batchOutput )
        {
            ...
            else
            {
                ...
                responseValidity.resize( static_cast<Eigen::Index>( searchPixelCount ), 1 );
                responseValidity.setZero();
                for( std::size_t region = 0; region < grids.size(); ++region )
                {
                    if( m_localPSFResponseRequired[region].size() != grids[region].searchPixelCount() )
                    {
                        throw mx::exception<verboseT>(
                            mx::error_t::sizeerr,
                            "P4 local-response requirements do not match detector-region geometry" );
                    }
                    for( std::size_t search = 0; search < grids[region].searchPixelCount(); ++search )
                    {
                        ...
                        localValidity( globalSearchIndex, 0 ) =
                            m_localPSFValidity[region]( static_cast<Eigen::Index>( search ),
                                                        static_cast<Eigen::Index>( output ) );
                        if( m_localPSFResponseRequired[region][search] != 0 )
                        {
                            responseValidity( globalSearchIndex, 0 ) = localValidity( globalSearchIndex, 0 );
                        }
                    }
                }
            }
                    ...
                    try
                    {
                        ...
                        if( differentiatedApproximation )
                        {
                            if( output >= refitDifferenceResponses.size() ||
                                ...
                                output >= refitDifferenceValidities.size() ||
                                sample >= refitDifferenceValidities[output].size() )
                            {
                                throw mx::exception<verboseT>(
                                    mx::error_t::sizeerr,
                                    "P4 refit-difference response sample is outside retained measurement state" );
                            }
                            sampledResponses[sample] = refitDifferenceResponses[output][sample];
                            sampledValidities[sample] = refitDifferenceValidities[output][sample];
                        }
                        else if( detectorLocalApproximation )
                        {
                            if( source >= searchPixelCount )
                            {
                                throw mx::exception<verboseT>( mx::error_t::sizeerr,
                                                               "P4 detector-local sample index is outside search "
                                                               "geometry" );
                            }
                            reconstructor.approximateSourceResponse(
                                sampledResponses[sample],
                                sampledValidities[sample],
                                localModels,
                                static_cast<Eigen::Index>( source ),
                                responseValidity( static_cast<Eigen::Index>( source ), 0 ) != 0 );
                        }
                        else
                        {
                            reconstructExact( sampledResponses[sample], sampledValidities[sample], source );
                        }
                    }
                    catch( ... )
                    {
                    ...
                    try
                    {
                        ...
                        if( radialModel )
                        {
                            ...
                            if( differentiatedApproximation )
                            {
                                ...
                                centerValidity( 0, 0 ) = combinedValidity( center, center );
                            }
                        ...
                        const int imageRow = static_cast<int>( sourceRow ) - coordinateOriginRow;
                        const int imageColumn = static_cast<int>( sourceColumn ) - coordinateOriginColumn;
...
void P4Reduction<realT, derotFunctObj, verboseT>::appendReductionHeader( fitsHeaderT &head ) const
{
        ...
        m_psfRefitContrast != 0 )
    {
        head.template append<realT>( "P4 PSF REFIT CONTRAST",
                                     m_psfRefitContrast,
                                     "paired refit half-amplitude in stored-template units" );
    }
    head.template append<std::string>( "P4 PSF SAMPLE EXCLUDED COUNT",
                                       std::to_string( m_psfSampleExcludedCount ),
                                       "candidate search pixels rejected near known planets" );
...
{
    if( m_automaticDerotationSize == 0 )
    {
        return;
    }
